...
:end:
** TODO Monadic text processing in haskell           :haskell:textprocessing:
:PROPERTIES:
...
*** Vấn đề
Việc xử lý text là công việc phức tạp và có nhiều nhu cầu đa dạng. Vì vậy, chỉ một bộ công cụ nhiều khi không đáp ứng được tất cả các nhu cầu của thực tế. Ta cần một bộ hàm số (text -> text) tiêu chuẩn giống như bộ hàm của Excel để làm hộ công cụ riêng.

Tuy nhiên, bên cạnh các hàm số sẵn có của các ngôn ngữ lập trình. Việc thiết kế các hàm đa dụng thực sự khó khăn, bởi vì phải tính đến hầu hết các trường hợp sử dụng hàng ngày của hàm số đó.
* Think                                                              :@think:
:properties:
:export_hugo_section: posts
:end:
...
:PROPERTIES:
:EXPORT_HUGO_PUBLISHDATE: [2023-12-09 Sat]
